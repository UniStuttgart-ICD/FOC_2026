...
- **Agent Turn**: One backend response to the latest user text; `AgentTurnProcessor` wraps backend output in Pipecat LLM frames and exposes explicit connect/disconnect lifecycle while Claude and Codex remain backend Adapters.
...
- **Safety Coverage**: Codex robot tools through `RobotMCPBridge` are locally enforced by `voice_runtime.robot_safety` before MCP execution. Direct Claude MCP is prompt-only and is not locally enforced unless a safe MCP proxy Adapter is added.
