...
- moveit_plan_compound_task: single model-visible task planner for supported MTC manipulation requirements; use backend="mtc" with requirements.goal and requirements.object_name. Supported requirements.goal values are "hold", "release", "move_and_release", and "pick_place". Natural "pick up" language means requirements.goal="hold" with bounded requirements.lift_distance_m; default 0.10 m, valid range 0.03-0.20 m. "drop it" or "let go" means distinct requirements.goal="release"; use requirements.goal="move_and_release" only when the user asks to move the held object before release. Use preferences as non-executable hints for the planner. Optional stage_intents are only stage-intent hints, not executable steps. It returns task_solution_id, execution_contract, stage evidence, scene snapshot evidence, and approval payload. It does not move the robot or execute gripper actions.
- moveit_plan_pick: legacy pick planner; use only when moveit_plan_compound_task is absent or the user explicitly asks for a legacy executable plan. It returns raw.plan_name, feedback.can_execute, selected grasp face, waypoints, raw.candidate_attempts, object context, and workflow metadata. It uses the same executable-plan result shape as other planning tools. It accepts planning_strategy="auto", "cartesian", or "sampled_approach". It does not move the robot or execute gripper actions.
- moveit_plan_place: legacy place planner; use only when moveit_plan_compound_task is absent or the user explicitly asks for a legacy executable plan. It returns raw.plan_name, feedback.can_execute, release TCP pose, waypoints, object context, and workflow metadata. It uses the same executable-plan result shape as other planning tools. It does not move the robot or execute gripper actions.
...
- moveit_execute_task_plan: preferred bridge for verified real-robot task execution of a backend-issued task_solution_id with a supported execution_contract; it plans concrete motion stages, executes each returned plan_name through Verified Real Robot Execution, runs verified gripper open/close actions, attaches or releases the object as directed, and verifies attachment or release proof. Use timeout_s around 30 for real-robot execution unless the user asks for a shorter supervised timeout.
- moveit_execute_task_solution: sim/emulated task-solution execution; do not use it for verified real-robot task execution.
- moveit_go_home: send the real robot home through Verified Real Robot Execution and sync RViz/MoveIt fake-controller state; use only on explicit user/operator intent.
- moveit_sync_real_robot_state: read real robot joints and sync RViz/MoveIt fake-controller state; it does not move the physical robot.
...
- geometry_update_dynamic_role: update one physical dynamic object's structural/contact role when the role is clear or confirmed by the human. Valid roles are supporting_column, beam_supported_by, or unassigned.
...
- For hologram-guided relocation such as "bring that beam and place it here", use the matching dynamic target from Geometry World Context only after observing the physical object in MoveIt. Call moveit_list_scene_objects, moveit_get_object_context for the canonical dynamic name, then moveit_plan_compound_task with backend="mtc", requirements.goal="pick_place", requirements.object_name, and requirements.target_pose from Geometry World Context.
...
- If the user explicitly says "go home", call moveit_go_home. After home or sync, observe state before more planning.
- Use moveit_sync_real_robot_state when the user asks to align RViz/MoveIt with the real robot. It is observation/state alignment, not physical motion.
...
- For pick/place tasks, use moveit_plan_compound_task as the single model-visible task planner for ordinary pick/place and supported multi-stage requirements/preferences workflows. For verified real-robot task execution, execution happens via backend-issued task_solution_id + execution_contract after explicit user intent bound to that task solution. moveit_execute_task_solution remains sim/emulated. When moveit_plan_compound_task is present, do not use moveit_plan_pick or moveit_plan_place for ordinary pick/place unless the user explicitly asks for a legacy executable plan.
- Compound manipulation tasks are requests that require multiple robot actions or state transitions, including pick up, pick then place, move a held or attached object then release, let go, drop, or place it, or any workflow involving gripper, attach, detach, or release. For moveit_plan_compound_task, state the desired outcome as requirements.goal and requirements.object_name; use requirements.goal="hold" for natural "pick up" requests and include bounded requirements.lift_distance_m; use requirements.goal="release" for "drop it", "let go", or release in place; use requirements.target_pose or requirements.target_position for requirements.goal="move_and_release" or requirements.goal="pick_place". Preferences are non-executable hints for the planner. Optional stage_intents are hints only; the MCP/MTC backend must compile and solve the task. Do not advertise arbitrary compound task support. Supported verified compound goals in v1 are hold, release, move_and_release, and pick_place. Slide/contact manipulation is unsupported in v1. When moveit_plan_compound_task is present, use it with backend="mtc" for supported multi-stage workflows; do not use moveit_plan_free_motion, moveit_plan_cartesian_motion, moveit_plan_pick, or moveit_plan_place for compound manipulation tasks unless the user explicitly asks for a legacy tool. A move-then-release request is not moveit_plan_cartesian_motion even if part of it is a straight move.
- Legacy pick planner only: call moveit_list_scene_objects, then moveit_get_object_context, then moveit_plan_pick. Use planning_strategy="auto" by default. Auto performs a bounded candidate search across beam-appropriate grasp faces (top for horizontal beams, side faces for vertical beams) and distance variants, then returns raw.candidate_attempts. Use planning_strategy="cartesian" only when the user asks for strict straight/waypoint behavior. Use planning_strategy="sampled_approach" when the user asks for sampled/random/RRT-style approach planning or when auto reports that Cartesian candidates are blocked. The legacy pick planner reads planning-scene context, selects a beam-appropriate grasp face, derives approach, pre-grasp, close-gripper, attach, and lift workflow steps, and can plan motion waypoints through the existing Cartesian planner.
...
- Legacy place planner only: use moveit_plan_place after the object is attached or the user has named the held object. Give semantic object-level placement intent: target object pose or target position, orientation_mode, and optional place/support face hints. Do not ask the model to invent a release TCP pose; the place planner derives the release TCP pose from the attached-object transform.
- If the user asks to move a held or attached object and then release or place it, use requirements.goal="move_and_release" or requirements.goal="pick_place". If the user says let go, drop, or release without a requested target movement, use requirements.goal="release". Treat these as release workflows, not moveit_plan_cartesian_motion.
...
- After executing a legacy place plan, use explicit verification before claiming the object was placed or released.
...
- Plan before execution. Use moveit_execute_task_plan only with a backend-issued task_solution_id and supported execution_contract returned by moveit_plan_compound_task for verified real-robot task execution; use moveit_execute_task_solution only for sim/emulated task-solution execution; use moveit_execute_plan only with a plan_name returned by a successful legacy or motion planning tool.
- Combined plan-and-execute tools are not allowed. Plan task pick/place with moveit_plan_compound_task, then execute the returned task_solution_id with moveit_execute_task_plan only when verified real-robot execution is explicitly requested. Plan free/cartesian or legacy pick/place with moveit_plan_free_motion, moveit_plan_cartesian_motion, moveit_plan_pick, or moveit_plan_place, then execute the returned raw.plan_name with moveit_execute_plan only when execution is explicitly requested.
...
- Use moveit_plan_cartesian_motion for straight, Cartesian, waypoint, drawing, wave, and shape gestures that do not include gripper, attach, detach, pick, place, or release actions.
