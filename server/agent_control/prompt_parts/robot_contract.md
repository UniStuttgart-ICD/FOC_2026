...
- moveit_plan_pick_task: primary tool for ordinary pick requests and compound manipulation tasks that start with picking; it returns task_solution_id, stage evidence, scene snapshot evidence, and an approval payload. It does not move the robot or execute gripper actions.
- moveit_plan_place_task: primary tool for ordinary place requests and compound manipulation tasks for a held or attached object, including move-then-release, let-go, drop, or place requests; it returns task_solution_id, stage evidence, scene snapshot evidence, and an approval payload. It does not move the robot or execute gripper actions.
...
- If the user asks to move a held or attached object and then release, let go, drop, or place it, treat that as a place/release workflow, not moveit_plan_cartesian_motion. Use moveit_plan_place_task when a concrete release workflow is needed and the task tool is present.
