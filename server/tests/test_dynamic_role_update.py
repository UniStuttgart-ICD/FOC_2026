...
def test_update_dynamic_role_sets_unassigned(tmp_path) -> None:
    ...
    assert _body(model, "dynamic_1")["state"]["role"] == {"type": "unassigned"}
...
def test_update_dynamic_role_sets_supporting_column(tmp_path) -> None:
    ...
    result = update_dynamic_role(
        "dynamic_01",
        {"type": "supporting_column", "supports": ["dynamic_03"]},
    ...
    assert _body(model, "dynamic_1")["state"]["role"] == role
...
def test_update_dynamic_role_sets_beam_supported_by(tmp_path) -> None:
    ...
    result = update_dynamic_role(
        "dynamic_3",
        {"type": "beam_supported_by", "supported_by": ["dynamic_01", "dynamic_02"]},
        "beam rests on two columns",
        model_path=model_path,
    )

    model = _read_model(model_path)
    role = {"type": "beam_supported_by", "supported_by": ["dynamic_1", "dynamic_2"]}
    assert result == {
        "ok": True,
        "object_name": "dynamic_3",
        "role": role,
        "physical_model_updated": True,
    }
    assert _body(model, "dynamic_3")["state"]["role"] == role
...
def _model() -> dict:
    return {
        ...
        "units": "meters",
        "bodies": [
            _model_body("dynamic_1", "left column"),
            _model_body("dynamic_2", "right column"),
            _model_body("dynamic_3", "span"),
        ],
        "operation_history": [
            {
